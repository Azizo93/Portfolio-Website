import React from "react";

const Workitem = ({ year, title, duration, details }) => {
  return (
    <ol className="flex flex-col md:flex-row relative border-l border-stone-200 pt-6">
      <li className="mb-16 ml-4">
        <div className="absolute w-3 h-3 bg-stone-200 rounded-full mt-1.5 -left-1.5 border-white" />
        <p className="flex flex-wrap gap-4 flex-row items-center justify-start text-xs md:text-sm py-2">
          <span className="inline-block px-2 py-1 font-semibold text-white bg-green-800 rounded-md">
            {year}
          </span>
          <span className="text-lg font-semibold text-green-800">{title}</span>
          <span className="my-1 text-sm font-normal leading-none text-stone-500">
            {duration}
          </span>
        </p>
        <p className="my-2 text-base font-normal text-stone-700 max-w-5xl">
          {details}
        </p>
      </li>
    </ol>
  );
};

export default Workitem;
